...
function acquireLock(projectDir, opName, retries = 10) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (_tryReclaimStaleLock(lockPath)) {
      ...
      try {
        const raw = fs.readFileSync(lockPath, 'utf8')
        console.error('DEBUG_READ1=' + raw)
        const info = JSON.parse(raw)
        holder = `${info.op} (PID ${info.pid})`
      } catch (e2) {
        console.error('DEBUG_CATCH1=' + e2.name + ':' + e2.message)
        try {
          const raw = fs.readFileSync(lockPath, 'utf8')
          console.error('DEBUG_READ2=' + raw)
          const info = JSON.parse(raw)
          holder = `${info.op} (PID ${info.pid})`
        } catch (e3) {
          console.error('DEBUG_CATCH2=' + e3.name + ':' + e3.message)
          ...
          try {
            ...
            console.error('DEBUG_CATCH3=' + e4.name + ':' + e4.message)
            holder = '锁文件损坏'
          }
        }
      }
