...
> 归档命名：`第XXX章-章节名--YYYYMMDD-HHMMSS--操作类型.md`（操作类型：replace / deleted / restore / write）
>
> **[操作日志规则]**：删除、导入（插入/替换模式）、恢复已删章节这三种高风险操作，必须使用操作日志 `<chapters_dir>/.__op_journal__.json` 跟踪进度。
>
> **启动前检查**：如果日志文件已存在，说明上次操作中断。读取日志内容，根据 `phase` 判断中断点，提示用户确认后执行对应的恢复/清理，不得跳过。
>
> **日志字段**：
> ```json
> {"op":"delete","ts":"2026-04-16T10:00:00","targets":["第003章-风起云涌.md"],"phase":"pre-archive","detail":{}}
> ```
> - `op`：操作类型（`delete` / `import-append` / `import-insert` / `import-replace` / `restore`）
> - `ts`：操作开始时间
> - `targets`：涉及的章节文件列表
> - `phase`：当前阶段（见下方 phase 表）
> - `detail`：阶段相关的补充数据（如归档路径、重编号映射等）
>
> **Phase 迁移与回滚**：
>
> | op | phase 顺序 | 中断时的恢复策略 |
> |---|---|---|
> | delete | `pre-archive` → `archived` → `refs-cleaned` → `renumbered` → ✅删除日志 | `pre-archive`：无需回滚（尚未修改）；`archived`：原文件已删但归档存在，从归档恢复原文件；`refs-cleaned`：归档恢复 + 提示用户手动检查元数据引用；`renumbered`：归档恢复 + 执行 renumber.js 恢复 + 提示检查引用 |
> | import-append | `pre-write` → `written` → ✅删除日志 | `pre-write`：无需回滚；`written`：删除新写入的章节文件 |
> | import-insert | `pre-write` → `written` → `renumbered` → ✅删除日志 | `pre-write`：无需回滚；`written`：删除新写入的章节文件；`renumbered`：执行 renumber.js 恢复 + 删除新文件 |
> | import-replace | `pre-archive` → `archived` → `replaced` → ✅删除日志 | `pre-archive`：无需回滚；`archived`：从归档恢复原文件；`replaced`：从归档恢复原文件 + 删除新文件 |
> | restore | `pre-copy` → `copied` → `renumbered` → ✅删除日志 | `pre-copy`：无需回滚；`copied`：删除 `.__restore__` 暂存文件；`renumbered`：执行 renumber.js 恢复 + 删除暂存文件 |
>
> **执行规范**：
> 1. 操作第一步修改之前写入日志（`phase` 设为该操作的起始阶段）
> 2. 每完成一个关键步骤，更新 `phase` 到下一阶段（覆盖写入同一文件）
> 3. 所有步骤成功完成后删除日志文件
> 4. 任何步骤失败时，保留日志文件不删，向用户报告当前 `phase` 和失败原因
...
**成功提示模板（统一口径）：**
- 成功提示统一遵循：`动作完成 ✓` → `影响范围` → `系统已自动处理` → `当前进度/当前位置` → `待检查（如有）`
- 若脚本返回 `warnings` 且 `ok=true`，成功提示里必须单独列出 `待检查：...`，不要把 warnings 混进正文描述

**失败提示模板（统一口径）：**
- 如果导入脚本返回锁占用 / 锁文件损坏：`导入未执行。当前项目正被其他写操作占用，或存在损坏锁（常见于异常中断 / 云盘同步占用）。可稍后重试；若确认当前没有其他写操作，再清理锁文件。`
- 如果导入脚本返回 journal / interrupted：`检测到上次导入/删除/恢复未完成，当前已进入保护状态。请先处理残留操作日志，再继续导入。`
- 如果导入脚本返回编码错误 / 非 UTF-8：`导入未执行。源文件编码不是 UTF-8，请先转换编码后重试。`
- 如果导入脚本返回 warnings 但 `ok=true`：`章节已导入，但以下项目可能仍需人工确认：...。必要时建议执行一次“一致性巡检”。`
...
9. **更新 `PROJECT.yaml`**（完成后删除操作日志文件）——**推荐使用脚本**：
   ```bash
   ...
   - **`current_chapter` 跟踪规则**：记录删除前 `current_chapter` 指向的章节文件名，在重编号完成后，查找该文件的新编号作为 `current_chapter` 的新值。如果该文件本身被删除了，则使用 `min(原 current_chapter, 重编号后现存最大编号)`
   - `total_chapters` 保持不变（仅表示计划总数，不需要因为删除而减少）
   - `updated` 更新日期
   - 重新评估 `active_characters` 和 `focus_plotlines`（使用滚动保留策略，而不是只按删除后当前章瞬时重算）
   - `last_action` 更新为 `{type: "delete", targets: ["原第XXX章", ...], timestamp: 当前时间}`（多章时用 targets 列表）
...
**成功提示模板（统一口径）：**
- 成功提示统一遵循：`动作完成 ✓` → `影响范围` → `系统已自动处理` → `当前进度/当前位置` → `待检查（如有）`
- 删除成功时，优先明确：`已删除哪些章`、`是否发生重编号`、`归档位置`、`当前进度`

**失败提示模板（统一口径）：**
- 如果删除脚本返回锁占用 / 锁文件损坏：`删除未执行。当前项目正被其他写操作占用，或存在损坏锁（常见于异常中断 / 云盘同步占用）。可稍后重试；若确认当前没有其他写操作，再清理锁文件。`
- 如果删除脚本返回 snapshot 失败：`删除已中止。删除前快照没有成功创建，所以系统拒绝继续删除，以免无法回滚。请先检查磁盘空间、目录权限或云盘占用。`
- 如果删除脚本返回 journal / interrupted：`检测到上次删除/导入/恢复未完成，当前已进入保护状态。请先处理残留操作日志，再继续删除。`
- 如果删除脚本返回 warnings 但 `ok=true`：`章节已删除，但以下项目可能仍需人工确认：...。建议执行一次“一致性巡检”。`
...
- **提示文案约定（统一口径）**：
  - 如果恢复后只能保证正文已恢复，但元数据可能不同步，明确说：`正文已恢复，但以下文件可能仍需人工确认：...。建议执行一次“一致性巡检”。`
  - 如果删除快照不存在或不可用，明确说：`找回章节可以继续尝试，但已无法精确回到删除前整个项目状态。`
  - 如果恢复失败且提示里出现锁文件损坏/同步盘冲突，应明确说：`检测到损坏锁，常见原因是异常中断或云盘同步占用。可稍后重试；若确认当前没有其他写操作，再清理锁文件。`
...
**成功提示模板（统一口径）：**
- 成功提示统一遵循：`动作完成 ✓` → `恢复来源/恢复方式` → `系统已自动处理` → `待检查（如有）`
- 如果用户执行的是删除快照精确回滚，优先明确：`已回到删除前项目状态`、`已恢复哪些文件范围`、`当前建议`（通常是继续写作或做一次一致性巡检）

**失败提示模板（统一口径）：**
- 如果恢复脚本返回锁占用 / 锁文件损坏：`恢复未执行。当前项目正被其他写操作占用，或存在损坏锁（常见于异常中断 / 云盘同步占用）。可稍后重试；若确认当前没有其他写操作，再清理锁文件。`
- 如果恢复脚本返回 journal / interrupted：`检测到上次删除/导入/恢复未完成，当前已进入保护状态。请先处理残留操作日志，再继续恢复。`
- 如果恢复脚本返回“归档文件不存在”或候选版本已失效：`恢复未执行。所选历史版本已不存在，或已被清理/移动。请重新选择可用版本。`
- 如果恢复成功但只能保证正文恢复：`正文已恢复，但以下文件可能仍需人工确认：...。建议执行一次“一致性巡检”。`
